...
Teixeira’s decoupling logic is that disruptors can “steal” a narrow stage of the customer value chain rather than replace the incumbent bundle (books/unlocking-the-customer-value-chain-chapter-1.md) (E15), and mobile + location data can plausibly reduce the time/effort cost embedded in the “product search” step of the bottled-water CVC (E9), but we do not yet have direct evidence that Liquid Death customers systematically fail at real-time availability discovery (E6-E13).

Strongest argument: A “Liquid Death Now” locator directly targets the CVC step labeled Product Search (E9) by reducing uncertainty and effort between need recognition and purchase (E8, E11) while leaving downstream fulfillment to existing retailers—matching Teixeira’s pattern of decoupling a discrete activity instead of rebuilding production/distribution (E15).
...
```mermaid
flowchart LR
    A1["<b>Step 1</b><br/>Recognize a hydration need<br/><i>self (consumer); context (workout, commute, meal, etc.)</i>"]
    A2["<b>Step 2</b><br/>Search for feasible hydration options available right now<br/><i>self (memory/habit); nearby availability via retail/vending/online; alternatives include tap water and other beverages</i>"]
    A3["<b>Step 3</b><br/>Evaluate and compare options (brand, price, convenience, perceived health, packaging)<br/><i>incumbent beverage brands and Liquid Death (branding/packaging cues); retailers (shelf placement, assortment, pricing)</i>"]
    A4["<b>Step 4</b><br/>Purchase the chosen product through an available channel<br/><i>retailers (grocery, convenience), vending, or online; Liquid Death and incumbents supply product into these channels</i>"]
    A5["<b>Step 5</b><br/>Consume the water and handle the packaging after use<br/><i>self (drinks); local waste/recycling systems (disposal); product packaging format influences the experience</i>"]
    A6["<b>Step 6</b><br/>Reflect on experience (including environmental impact) and decide whether to repurchase<br/><i>self (preference/loyalty formation); brand messaging and past experience influence recall and loyalty</i>"]
    A1 --> A2
    A2 --> A3
    A3 --> A4
    A4 --> A5
...
```mermaid
flowchart TD
    S1["<b>PRESERVE</b><br/>launch an ultra-light MVP (mobile web + QR + SMS) that answers only one job—nearest place to buy now—focused on the Product Search step (E3, E9, E11)."]
    S2["Instrument unit economics before scaling: measure incremental conversion proxies (store-locator click-to-navigation rate, repeat usage, and geo-level sales lift where available) to test whether incremental gross profit from added units pla…"]
    S3["<b>LIGHT</b><br/>Increase data quality with low-risk trust mechanisms (layer a): add venue-verified availability where partners cooperate plus user-reported “confirmed in stock” with recency ranking to reduce false positives and time waste in the search st…"]
    S4["<b>MEDIUM</b><br/>Only after demonstrating measurable lift, move to medium-responsibility intermediation (layer b): add retailer-specific offers/coupons or “reserve for pickup” referrals where the retailer remains merchant-of-record, keeping Liquid Death ou…"]
    S5["If (and only if) the locator demonstrates repeat behavior and retailer demand, create a B2B insight product (heatmaps of demand/search) for distribution partners—explicitly as an add-on, not the primary value capture path (hypothesis; not …"]
    S1 --> S2
    S2 --> S3
    S3 --> S4
    S4 --> S5
