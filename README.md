...
The `cases/` directory is the publishable case-study archive for Path C. It now
separates the historical v0 ungrounded baselines from grounded cases built with
Tavily retrieval and source lists verified against GPT Researcher's
`research_sources`.

See [cases/README.md](cases/README.md) for the case index. Each archived case
has `run/`, `review.md`, `baseline_metrics.json`, and grounded-era provenance
