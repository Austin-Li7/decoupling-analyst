...
The `cases/` directory is the publishable case-study archive for Path C:
baseline runs, human reviews, and lightweight metrics that make methodology
quality auditable over time.

See [cases/README.md](cases/README.md) for the case index. Each archived case
has `run/`, `review.md`, and `baseline_metrics.json`; `review.md` is written by
the human reviewer, not auto-generated by the pipeline.
